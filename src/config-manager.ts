...
if (process.env.PN_ACT_CONFIG_EMAIL_SECURE) {
	if (
		process.env.PN_ACT_CONFIG_EMAIL_SECURE !== 'true' &&
		process.env.PN_ACT_CONFIG_EMAIL_SECURE !== 'false'
	) {
		LOG_ERROR(
			`PN_ACT_CONFIG_EMAIL_SECURE must be either true or false, got ${process.env.PN_ACT_CONFIG_EMAIL_SECURE}`
		);
...
if (!config.email.auth.user) {
	LOG_WARN('Failed to find email account username. Disabling feature. To enable feature set the PN_ACT_CONFIG_EMAIL_USERNAME environment variable');
	disabledFeatures.email = true;
}
...
if (
	config.datastore.signature_secret.length !== 32 ||
	!hexadecimalStringRegex.test(config.datastore.signature_secret)
) {
